...
fn temporal_dijkstra(graph: &AnnexTimeVaryingGraph, start: Node, end: Node, max_time: u64, time: u64, max_node_index: u64, distance_matrices: &mut Vec<DistanceMatrix>) -> Path {
    ...
    while let Some(DijkstraNode { node, cost }) = heap.pop() {
        if visited[node as usize] {
            distance_matrices[time as usize][start as usize][node as usize] = cost;
            continue;
        }
        visited[node as usize] = true;
        for edge in &graph.edges {
    ...
    for (from, weight, to) in &steps {
        ...
        distance_matrices[time as usize][start as usize][*to as usize] = sum as u32;
        distance_matrices[time as usize + sum as usize][*from as usize][*to as usize] = sum as u32;
    }
    return Path {
        from: start,
        to: end,
        ...
        for i in 0..graph.dst_mat_undel.len() {
            for j in 0..graph.dst_mat_undel[i].len() {
                if graph.dst_mat_del[t as usize][i][j] == 0 {
                    ...
                    let dst_matrices = unsafe { std::mem::transmute::<&mut Vec<DistanceMatrix>, &mut Vec<DistanceMatrix>>(&mut graph.dst_mat_del) };
                    let path = temporal_dijkstra(graph, i as u64, j as u64, graph.max_time, t, max_node_index, dst_matrices);
                    print_path(&path);
                    /*let sum = path.steps.iter().map(|(_, w, _)| w).sum::<u32>();
                    // TODO TAG PROUT
