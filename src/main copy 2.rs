...
fn temporal_dijkstra(graph : &AnnexTimeVaryingGraph, source : u64, destination : u64, time : u64, distance_matrices : *mut Vec<Vec<Vec<u64>>>, max_time : u64) -> u64 {
    ...
    for _ in 0..graph.edges.len() {
        ...
        unsafe {
            let distance_matrices = distance_matrices as *mut Vec<Vec<Vec<u64>>>;
            (*distance_matrices)[time as usize][source as usize][min_index as usize] = distances[min_index];
        }
        for i in 0..graph.edges.len() {
...
fn full_parallel_johnson(graph : &AnnexTimeVaryingGraph) -> Vec<DistanceMatrix> {
    ...
    let ptr_distance_matrices = &mut distance_matrices as *mut Vec<Vec<Vec<u64>>> as usize;
    ...
    (0..graph.max_time).into_par_iter().for_each(|t| {
        (0..graph.edges.len()).into_par_iter().for_each(|i| {
            (0..graph.edges.len()).into_iter().for_each(|j| {
                if i == j {
                    return;
                }
...
fn johnson(graph : &AnnexTimeVaryingGraph) -> Vec<DistanceMatrix> {
    ...
    let distance_matrices = unsafe { std::mem::transmute::<Vec<Vec<Vec<u64>>>, Vec<DistanceMatrix>>(distance_matrices) };
...
/*
The difference in results between full_parallel_johnson, time_parallel_johnson, and johnson is likely due to race conditions in the full_parallel_johnson function.

In full_parallel_johnson, you are using into_par_iter() to create parallel iterators for time, source, and destination nodes. This means that multiple threads can be executing temporal_dijkstra and updating the distance_matrices simultaneously. If two threads are trying to update the same element of distance_matrices at the same time, you can get inconsistent results.

In contrast, time_parallel_johnson only parallelizes the outer loop over time, while the loops over source and destination nodes are serial. This ensures that only one thread is updating any given distance_matrices[t] at a time, preventing race conditions.

The johnson function is fully serial, so it does not have any race conditions.

To fix this, you need to ensure that updates to distance_matrices are properly synchronized in full_parallel_johnson. One way to do this is to use a thread-safe data structure, such as a Mutex, to protect distance_matrices. However, this can significantly impact performance. Another option is to redesign your algorithm to avoid shared mutable state. For example, you could have each thread compute a separate distance matrix, and then combine them at the end.
