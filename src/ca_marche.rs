        ...
        for i in 0..graph.dst_mat_undel.len() {
            for j in 0..graph.dst_mat_undel[i].len() {
                if graph.dst_mat_undel[i][j] == 0 && i != j {
                    graph.dst_mat_undel[i][j] = temporal_dijkstra(graph, i as u64, j as u64, t).steps.iter().map(|(_, w, _)| w).sum();
                }
            }
        }
    }
...
fn main() {
    ...
    println!("Sum: {}, Reachables: {}", sum, reachables);
}
