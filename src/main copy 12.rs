...
fn invalidate_deleted_edges_new(paths: &Vec<CorrelatedPathsTwo>, dst_mat_undel: &DistanceMatrix, max_time: u64, dst_mat_del: &mut Vec<DistanceMatrix>) {
    // For each path, if it contains a deleted edge, all the subpaths have to be invalidated
    for path in paths {
        let main_path = &path.main_path;
        let mut sum = 0;
        for i in 0..main_path.steps.len() {
            let (from, weight, to) = &main_path.steps[i];
            // see if the edge was deleted at sum
            if dst_mat_undel[*from as usize][*to as usize] == u32::MAX {
                // invalidate the distance matrix
                dst_mat_del[sum as usize][*from as usize][*to as usize] = 0;
                // for the rest of the path, we have to invalidate the distance matrix
                for j in i+1..main_path.steps.len() {
...
fn graph_to_temporal(graph: &Graph, max_time: u64, deleted_edges: &Vec<DeletedLink>) -> AnnexTimeVaryingGraph {
    ...
    benchmark!("invalidate_deleted_edges", invalidate_deleted_edges_new(&paths, &dst_mat_undel, max_time, &mut dst_mat_del));
