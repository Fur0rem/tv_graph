...
fn from_shortest_paths(paths: &Vec<Path>, nodes: &Vec<Node>, max_node_index: u64, max_time: u64) -> DistanceMatrix {
...
fn recompute_all_distance_matrix(graph: &mut AnnexTimeVaryingGraph) {
    ...
    let max_node_index = graph.nodes.iter().max().unwrap() + 1;
    for t in 0..graph.max_time {
...
fn main() {
    ...
    let deleted_edges = vec![(0, 1, 0), (0, 1, 1)];
    let nodes : Vec<Node> = (0..201).collect();
    ...
    let deleted_edges = vec![(0, 1, 0), (0, 1, 1)];
    let mut nodes : Vec<Node> = edges.iter().map(|e| e.from).collect();
    nodes.extend(edges.iter().map(|e| e.to));
    nodes.sort();
    nodes.dedup();
    let max_node_index = nodes.iter().max().unwrap() + 1;
    let max_time = 3;
    let graph = Graph {
        max_node_index,
        nodes,
